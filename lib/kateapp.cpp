...
Q_LOGGING_CATEGORY(LOG_KATE, "tukwriter", QtWarningMsg)
...
void KateApp::initPreApplicationCreation(bool detach)
{
    ...
    if (getuid() == 0) {
        setlocale(LC_ALL, "");
        bindtextdomain("tukwriter", KDE_INSTALL_FULL_LOCALEDIR);
        if (!qEnvironmentVariableIsEmpty("SUDO_USER")) {
            ...
            std::cout << dgettext("tukwriter", message.untranslatedText()) << std::endl;
            exit(EXIT_FAILURE);
